...
namespace franky {
  struct MotionGenerator {
    ...
    static std::tuple<std::array<double, 7>, std::array<double, 7>, std::array<double, 7>>
    getInputLimits(RobotType *robot, const MotionData &data) {
      return getInputLimits(robot, Waypoint(), data);
    }

    template<class RobotType>
    static std::tuple<std::array<double, 7>, std::array<double, 7>, std::array<double, 7>>
    getInputLimits(RobotType *robot, const Waypoint &waypoint, const MotionData &data) {
      ...
      auto max_velocity = MotionGenerator::VectorCartRotElbow(
          translation_factor * waypoint.velocity_rel * data.velocity_rel * robot->velocity_rel *
      ...
      auto max_acceleration = MotionGenerator::VectorCartRotElbow(
          translation_factor * derivative_factor * data.acceleration_rel * robot->acceleration_rel *
      ...
      auto max_jerk = MotionGenerator::VectorCartRotElbow(
          translation_factor * std::pow(derivative_factor, 2) * data.jerk_rel * robot->jerk_rel *
    ...
    template<class RobotType>
    static void setInputLimits(ruckig::InputParameter<7> &input_parameters, RobotType *robot, const Waypoint &waypoint,
                               const MotionData &data) {
      std::tie(input_parameters.max_velocity, input_parameters.max_acceleration,
               input_parameters.max_jerk) = getInputLimits(robot, waypoint, data);
