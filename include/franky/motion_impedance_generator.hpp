...
namespace franky {
  ...
  struct ImpedanceMotionGenerator : public MotionGenerator {
    ...
    explicit ImpedanceMotionGenerator(RobotType *robot, const Affine &frame, ImpedanceMotion &motion, MotionData &data)
      ...
      initial_affine = Affine(Eigen::Matrix4d::Map(initial_state.O_T_EE.data()));
      position_d = Eigen::Vector3d(initial_affine.translation());
      orientation_d = Eigen::Quaterniond(initial_affine.rotation());
    }

    void init(const franka::RobotState &robot_state, franka::Duration period) {
      Affine transform(Eigen::Matrix4d::Map(robot_state.O_T_EE.data()));
    ...
    franka::Torques operator()(const franka::RobotState &robot_state, franka::Duration period) {
      ...
      switch (motion.target_motion) {
        case ImpedanceMotion::TargetMotion::Exponential: {
          position_d =
              motion.exponential_decay * motion.target.translation() + (1.0 - motion.exponential_decay) * position_d;
          orientation_d = orientation_d.slerp(motion.exponential_decay, Eigen::Quaterniond(motion.target.rotation()));
        }
          break;
        case ImpedanceMotion::TargetMotion::Linear: {
          ...
          if (transition_parameter <= 1.0) {  // [ms] to [s]
            Affine absolute_target = motion.linear_motion.relative_target * initial_affine;
            Eigen::Quaterniond q_start(initial_affine.rotation());
            Eigen::Quaterniond q_end(absolute_target.rotation());
            auto init_trans = initial_affine.translation();
            motion.target.translation() = init_trans +
                                          transition_parameter * (absolute_target.translation() - init_trans);
            motion.target.linear() = q_start.slerp(transition_parameter, q_end).toRotationMatrix();
            position_d = motion.target.translation();
            orientation_d = Eigen::Quaterniond(motion.target.rotation());
          } else if (motion.linear_motion.finish_after &&
                     transition_parameter > motion.linear_motion.finish_wait_factor) { // Wait a bit longer to stop
