...
namespace franky {
  ...
  void Path::init_path_points(const std::vector<Waypoint> &waypoints) {
    ...
    for (size_t i = 1; i < waypoints.size() - 1; i += 1) {
      if (waypoints[i].blend_max_distance > 0.0) {
        ...
        auto blend = std::make_shared<QuarticBlendSegment>(left->start, lm, right->start, rm, left->get_length(),
                                                           waypoints[i].blend_max_distance, s_abs_max);
  ...
  Vector7d Path::max_pddq() const {
    ...
    for (size_t i = 0; i < 7; i += 1) {
      auto max_ptr = *std::max_element(segments.begin(), segments.end(),
                                       [&](std::shared_ptr<Segment> l, std::shared_ptr<Segment> r) {
