...
JointMotion::JointMotion(const std::array<double, 7> q_goal): q_goal(q_goal.data()) {
...
void JointMotion::update(Robot* robot, const Affine& frame, const MotionData& motion_data) {
  ...
  dq_max_ *= robot->velocity_rel * motion_data.velocity_rel;
  ddq_max_start_ *= robot->acceleration_rel * motion_data.acceleration_rel;
  ddq_max_goal_ *= robot->acceleration_rel * motion_data.acceleration_rel;
}
...
franka::JointPositions JointMotion::operator()(const franka::RobotState& robot_state, franka::Duration period) {
  ...
  if (time == 0.0) {
    ...
    delta_q_ = q_goal - q_start_;
    calculateSynchronizedValues();
  }

  Vector7d delta_q_d;
  bool motion_finished = calculateDesiredValues(time, &delta_q_d);
